...
export function getPersistedState(state: AppState) {
  ...
  return {
    settings,
    params: state.params,
    ...(settings.persistInputOnRestart && (state.appMode === 'gallery' || state.appMode === 'sop' || galleryInputDraft)
      ? {
...
function saveGalleryInputDraft(state: Pick<AppState, 'appMode' | 'galleryInputDraft' | 'prompt' | 'inputImages' | 'maskDraft' | 'maskEditorImageId'>) {
  if (state.appMode !== 'gallery' && state.appMode !== 'sop') return state.galleryInputDraft
...
function syncActiveInputDraft<T extends Partial<AgentInputDraft>>(
  ...
  if (state.appMode === 'gallery' || state.appMode === 'sop') {
    return {
      ...patch,
  ...
  persist(
    (set, get) => ({
      ...
      setAppMode: (mode) => {
        ...
        const nextMode: AppMode = mode === 'sop' ? 'sop' : 'gallery'
        set((state) => ({
          appMode: nextMode,
          agentInputDrafts,
          galleryInputDraft,
