...
describe('summarizeGenerationError', () => {
  it('summarizes provider JSON server errors in Chinese and keeps request id', () => {
    const message = summarizeGenerationError(
      JSON.stringify({
        error: {
          type: 'server_error',
          code: 'server_error',
          message:
            'An error occurred while processing your request. You can retry your request, or contact us through our help center at help.openai.com if the error persists. Please include the request ID 0ff61b97-2a54-4948-aa82-d4b06dc98f1c in your message.',
          param: null,
        },
      }),
    )
