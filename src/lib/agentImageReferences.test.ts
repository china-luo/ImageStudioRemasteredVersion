...
describe('agent image references', () => {
  it('resolves previous round image references from visible text', () => {
    const rounds = [round({ index: 1, outputTaskIds: ['task-a'] }), round({ index: 2, outputTaskIds: ['task-b'] })]
  ...
  it('keeps previous round image numbering stable after a task is removed', () => {
    ...
    expect(
      resolveAgentPromptImageReferences('参考 @第1轮图1 和 @第1轮图2', rounds, [task('task-live', ['image-live'])]),
    ).toEqual(['image-live'])
  })
