...
describe('Amazon planner API lifecycle', () => {
  ...
  it('normalizes a package contents array returned by a non-strict chat API', async () => {
    const payload = createPlannerPayload()
    const responsePayload = {
      ...payload,
      product: {
        ...payload.product,
        packageIncludes: ['mug', ' lid ', '', 2],
      },
    }
    vi.stubGlobal(
      'fetch',
      vi.fn(
        async () =>
          new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify(responsePayload) } }] }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          }),
      ),
    ...
    expect(result.parsed.inferred.packageIncludes).toBe('mug\nlid')
    expect(result.plans).toHaveLength(7)
  })
